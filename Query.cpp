...
// QueryResult left_result = text.query(left_query);
    // QueryResult right_result = text.query(right_query);
    // auto r_lines = make_shared<set<line_no>>();
    // auto l_lines = make_shared<set<line_no>>();
    // auto ret_lines = make_shared<set<line_no>>();

    // r_lines->insert(right_result.begin(), right_result.end());
    // l_lines->insert(left_result.begin(), left_result.end());

    // int sum = 0;

    // auto l = l_lines->begin();
    // auto r = r_lines->begin();

    // while(l != l_lines->end() || r != r_lines->end()){
    //   if(abs(*l-*r)){
    //     sum++;
    //     ret_lines->insert(*l);
    //     ret_lines->insert(*r);
